...
```
For the RSPD, the logistic model achieved a p-value of 0.108, and could therefore not be rejected. Based on the available data, this model represents the best explanation of the data. For the SSPD all of the standard models could be rejected.
...
A notable pattern in Figure \@ref(fig:spd_mc) is the lack of data in the RSPD before c. 8500--8300 BCE while the SSPD has a marked peak around 9000 BCE. Comparing the the directions of the standard models in the figure, the logistic model fit to the RSPD in Figure \@ref(fig:spd_mc) indicates an overall uniform distribution in the frequency of radiocarbon dates following the initial jump just before 8000 BCE. This is contrasted to a general decrease in shoreline dated sites after the initial rapid increase from around 9500 BCE. Although they could be rejected as explaining the data, this is indicated by the decaying exponential function (Figure \@ref(fig:spd_mc)A) and the initial under-prediction followed by regions of over-prediction with the logistic and uniform models (Figure \@ref(fig:spd_mc)B and C).

It is worth noting that there appears to be a corresponding peak in both proxies just after 8000 BCE and similarly a drop just before 6000 BCE. However, given that the logistic model cannot be rejected for the RSPD, it is inappropriate to interpret any deviations from the Monte Carlo envelope, and no analytical weight can be placed on these deviations -- at least given the present data and modelling efforts.


# Discussion

It is established that Stone Age sites in the region date to the period from before the earliest ^14^C-dates, and at least from as far back as around 9300 BCE [e.g. @damlien2018b; @glorstad2016]. While the SSPD starts from c. 9500 BCE, the RSPD show no evidence of human activity until c. 8500–8300 BCE. What has caused the lack of ^14^C-dates from this earliest period has been given many possible explanations through the years. These range from from taphonomic loss to diverse environmental and/or cultural factors, such as the use of shrubs for fuel or use of rapidly deteriorating seal blubber for burning, a preference for consuming raw, dried or fermented food, and possibly a lack of reliable access to firewood [e.g. @damm2022]. While taphonomic and ecological factors cannot be written off, we view it as likely that the initial jump at the start of RSPD combined with the magnitude of the SSPD in the period preceding this, suggest that cultural factors is a central driver behind the observed pattern. 

The considered models indicate that after the initial jumps, some process of decrease in shoreline dated site frequency occurs throughout the period, contrasted by a general stationary frequency of ^14^C-dated material. An immediate explanation of this mismatch could be that the SSPD largely reflects variations in land-use and mobility patterns, while the RSPD mainly reflects population numbers. A common interpretation of the Early Mesolithic in the region is that the population was low in numbers but highly mobile, reflected in large numbers of short lived sites along the coast [e.g. @bjerck2017]. This is commonly seen in relation to a rapid colonisation of the Norwegian coastline and homogeneous lithic technology over large parts of Scandinavia in this period. High mobility has been seen as necessary both to maintain demographic viability among colonising groups and for it to be possible to maintain this technological uniformity over large areas [@rowley-conwy2017]. The initial peak in the SSPD could thus be congruent with a process of rapid colonisation of the Norwegian coast, coupled with a high degree of mobility, leading to the overall high site-count in the earliest period. At the same time, the low/lacking signal in the RSPD for this earliest period could be taken as evidence for low population numbers during the first settlement period following the deglaciation. 

@berg-hansen2022 have argued that there is an increase in sites between c. 8000--7000 BCE in the northern part of the study area of this paper. However, their analysis covers a limited spatial area and is based on the aggregation of point-based estimates of shoreline dates, which are aggregated within periods of variable length. Apart from the limitations with using point-based estimates for shoreline dates, the use of disjoint time-intervals of variable duration could cause issues for comparing the relative counts across these units [@bevan2021]. In contrast to their findings, the analysis undertaken here instead indicates a rapid increase followed drop in the number of sites through this time-interval, possibly corresponding with a shift to reduced residential mobility [@roalkvam2022; @damlien2018b; cf. @binford1980].

As evidenced by both genetic data and technological analyses of lithic inventories, the initial phase of human colonisation from the south appears to be followed by an influx, and possible mixing, of people from the east at the transition to the Middle Mesolithic period around 8300 BCE [@gunther2018; @manninen2021]. During the first part of the Middle Mesolithic, it has been suggested that the mobility patterns shifted to a more stable settlement, confined to larger regions [@damlien2018b]. If this narrative is coupled with the data observed here, these migratory events would appear to result in a relatively sudden drop in site-count as indicated in the SSPD. The end of this drop corresponds to the first ^14^C-dates in the RSPD around 8300 BCE -- giving further credence to the suggestion that the appearance of material to be ^14^C-dated is mainly related to cultural and demographic processes rather than taphonomic factors. 

The development after 4000 BCE should be treated with further care, following from questions concerning the reliability of shoreline dating after 4000 BCE [cf. @roalkvam2023].
...
Jeg fikk ADMUR til å funke med C14-dateringer. Figuren under viser at den logaritmiske modellen (blå kurve, samme som i Figur 5 over) ifølge BIC passer best av alternativene , og Monte Carlo-simuleringen over viser at den ikke kan avvises som forklarende for dataen. Eller med andre ord, dataen ville kunne forventes under den logaritmiske utviklingen. CPL modellering av strandlinjedateringene sliter jeg imidlertid mer med. Modellene opp til 4-CPL ser rimelige ut, men toppen på 5-CPL (sort kurve) som ligger litt før 5000 f.Kr. virker feil. I tillegg indikerer BIC-verdiene at denne passer best til dataen, som også virker rart gitt at den avviker såpass fra SSPDen. 

```{r cpl_rcarbon, echo = FALSE, out.width = "500px", fig.align="center", regfloat = TRUE}
# fig.cap = "Model comparison, summed probability of radiocarbon dates.",
knitr::include_graphics(here("analysis/figures/cplrcarbon.png"))
```


```{r cpl_shore, echo = FALSE, out.width = "500px", fig.align="center", regfloat = TRUE}
# fig.cap = "Model comparison, summed probability of shoreline dates.",
knitr::include_graphics(here("analysis/figures/cplshore.png"))
```
