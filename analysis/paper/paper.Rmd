...
The procedure outlined above represent the standard approach in efforts to model population dynamics based on RSPDs. This can be helpful for getting an impression of the overall developments of population proxies, and is an especially useful first step in the treatment of shoreline dated sites done here, as few assumptions concerning this overall development could be made. However, standard models frequently used to evaluate RSPDs are consistently being rejected in the literature and it is worth underscoring, as mentioned above, that deviations from these rejected null-models do not in themselves allow for any interpretations of the local deviations, nor a subsequent direct analysis of the SPDs themselves.
...
With an estimated trajectory of shoreline displacement at the site to be dated, the elevation of the site can be drawn on to find a likely date for when it was in use, under the assumption that it was in use when located on or close to the shoreline. Figure \@ref(fig:shoredate)C indicates where the mean elevation of the polygon representing the example site intersects the interpolated displacement curve. As it can reasonably be assumed that a site was not in use when located under water, this effectively represents a *terminus post quem* (TPQ) date. A common approach to shoreline dating has been to find where the elevation of the site intersects the mean of the upper and lower limit of the displacement curve, and then adding a somewhat arbitrary error of $\pm$ 100 years to this point-estimate. However, TPQ dates that do not account for the likely distance between site and sea when the site was in use limits the further inferential steps that can be taken. Furthermore, the more common practice of adding a constant error to a point-estimate does not take into account that the rate of shoreline displacement varies both spatially and temporally, and therefore introduces bias to the result on both the spatial and temporal scale, while also overestimating the precision of the method in most cases. 
...
```{r shoredate, echo = FALSE, fig.cap = "A) An example site (Askeladden ID 111967-1) relative to the isobases of the displacement curves.  B) The geologically derived shoreline displacement curves for the region and the curve interpolated to the example site. C) The black line on the y-axis represents the elevation of the site. This crosses the interpolated displacement curve at 7680 and 7490 BCE, which thus in practice gives an estimate of when the location of the site emerged from the sea, giving a \\textit{terminus post quem} date. D) The shoreline dating approach used for this study, where the gamma distrbution on the y-axis describes the likely vertical relationship between the shoreline and the site when it was in use.",  out.width = "500px", fig.align="center", regfloat = TRUE}
...
For each dating method, the relative performance of the exponential, logistic and uniform models was compared. This was done by finding the overall relative log-likelihoods of the models using the *loglik* function from *ADMUR*. For any given model, this involves finding the scalar product for each shoreline date against the probability of the null-model, and in the case of the ^14^C-dates, each site-phase. The overall model likelihood is the product of these individual probability and is not, critically [@timpson2021, 3], found by using the final SPD. From these likelihoods, the Bayesian (or Schwarz) information criterion (BIC) was then found for each model. This penalises the models for the number of parameters in use, to avoid over-fitting, and can be used to compare the relative performance of the models. 
...
All of the standard models could be rejected for the SSPD. Of the contrasted models, the BIC values indicate that the exponential model is the best candidate. Its negative direction indicates that some overall process of reduction in the frequency of shoreline dated sites occurs over time, while the rejection of the model and clear deviations from the exponential patterns indicates that this model does not account for the entire development. Some significant deviations occurs with the model over-predicting the frequency of sites in the period from c. X--9000 BCE, followed by a significant positive peak just after 9000 BCE. The process of decline is then interrupted by positive deviations at, and with a smaller deviation around 7000 BCE. This is closely followed by a negative deviation of more magnitude than that expected under the exponential model between X and X BCE. Finally, a positive deviation occurs again at X--5000 BCE. While all of the above deviations should also be considered with caution, questions concerning the reliability of shoreline dating after 4000 BCE [cf. @roalkvam2023] and the small magnitude of the negative deviations that occur around 3000 BCE means that are disregarded here.
...
```{r bic, echo = FALSE, fig.cap = "Bayesian information criterion for the standard models fit to the SPDs.", out.width = "500px", fig.align="center", regfloat = TRUE}
knitr::include_graphics(here("analysis/figures/bic.png"))
```

# Discussion

It is established that Stone Age sites in the region date to the period from before the earliest ^14^C-dates, and at least from as far back as around 9300 BCE [e.g. @damlien2018b; @glorstad2016]. While the SSPD achieved here starts from c. 9500 BCE, the RSPD shows no evidence of human activity until c. 8600 BCE. What has caused the lack of ^14^C-dates from this earliest period has been given many possible explanations through the years. These range from taphonomic loss to diverse environmental and/or cultural factors, such as the use of shrubs or rapidly deteriorating seal blubber as fuel for burning, a preference for consuming raw, dried or fermented food, and possibly a lack of reliable access to firewood [see @damm2022]. While taphonomic and ecological factors cannot be excluded as part of the picture, we view it as likely that the rapid growth at the start of RSPD and the magnitude of the SSPD in the period preceding this suggest that cultural and demographic factors are central drivers behind the observed pattern. 

The considered models also indicate that after the initial jumps of each proxy, some process of decrease in shoreline dated site frequency occurs throughout the period, contrasted by a general stationary frequency of ^14^C-dated material. An immediate explanation of this mismatch could be that the SSPD is more heavily impacted by variation in land-use and mobility patterns, while the RSPD mainly reflects population numbers.

A common interpretation of the Early Mesolithic in the region is that the population was small but highly mobile, reflected in large numbers of short-lived sites along the coast [e.g. @bjerck2008; @bjerck2017]. This is commonly seen in relation to homogeneous lithic technology over large swathes of the Scandinavian Peninsula in this period, and what is believed to have been a rapid colonisation of the Norwegian coastline [e.g. @bang-andersen2012; @bjerck2009; @fuglestvedt2012]. High mobility has been seen as necessary both to maintain demographic viability among colonising groups, and for it to be possible to maintain this technological uniformity over large areas [@rowley-conwy2017; also discussed by @berg-hansen2018]. The initial peak in the SSPD could thus be congruent with a process of rapid colonisation of the Norwegian coast, coupled with a high degree of mobility, leading to the overall high site-count in the earliest period. The lacking signal in the RSPD for the first settlement period following the deglaciation could in this view reflect mobility patterns characterised by short occupational duration and corresponding pyrotechnology based on small, short-lived fires [cf. @damm2022] and low population numbers. 

Following this earliest phase of rapid increase in shoreline dated sites, there a drop from this initial peak starts around 8800 BCE and concludes around 8000 BCE. It has previously been suggested that a change in mobility patterns occurs from some time around 8300 BCE, involving a reduction in residential mobility [e.g. @damlien2018b; @roalkvam2022]. Furthermore, both genetic data and technological analyses of lithic inventories indicates that the initial phase of human colonisation from the south is followed by an influx, and possible mixing, of people from the east around the same time [@günther2018; @manninen2021]. If this narrative is coupled with the data observed here, these migratory events could appear to result in a relatively sudden drop in site-count as indicated in the SSPD, which in turn could be related to a change in mobility patterns. The end of this drop also roughly corresponds to the first ^14^C-dates in the RSPD around 8300 BCE, leading to the logistic growth that plateaus in the first centuries after 8000 BCE. This logistic growth in the RSPD has a corresponding positive deviation in the SSPD, which could indicate that this demographic signal is reflected in the frequency of shoreline dated sites as well.

@berg-hansen2022 argued that there is an increase in sites between c. 8000--7000 BCE in the northern part of the study area of this paper. Their analysis covers a more limited area and is based on the aggregation of point-based estimates of shoreline dates, which are aggregated within periods of variable length. Apart from the limitations with using point-based estimates for shoreline dates, the use of disjoint discrete time-intervals of variable duration could cause issues for comparing the relative counts across these units [@bevan2021]. However, in line with the findings of @berg-hansen2022, the analysis undertaken here indicates a positive and quite marked reversal of the declining trend both around 8000--7500 BCE and with a smaller magnitude around 7100--6900 BCE.

Subsequently, the SSPD deviates negatively from the exponential decay from c. 6800 to c. 5800 BCE, a decrease that was also noted by @berghansen2022. This is followed by a positive deviation just before the 4th millennium BCE. The negative deviation could possibly been seen in relation to an increase in inland activity in southern Norway that occurs around the same time, which is argued to reach its peak around 6000 BCE. It is also worth noting that there appears to be a corresponding negative deviation from the logistic null model in the RSPD just before 6000 BCE. However, given that the model could not be rejected for the RSPD, it is inappropriate to interpret any deviations from the Monte Carlo envelope, and so no analytical weight can be placed on the deviations in the RSPD, given the present data. 

@boaz1999 related the development in inland activity to a separation of inland and coastal populations in this period. This is followed by the onset of the classic Nøstvet phase in the middle of the 6th millennium BCE  [@reitan2022], which has traditionally been seen as characterised by a reduction in residential mobility, and a specialisation towards fishing. This also coincides with what @fuglestvedt2018[17] has been termed \'the great wave\' of rock art in south-eastern Norway [@fuglestvedt2018, 42--46; @glorstad2010, 216--233]. The developments that might be reflected in the positive deviation in the SSPD. 

Given the rejected null-models, the stable development in RSPD over this period, and the limited sample of ^14^C-dates, it is, however, difficult to  

Given the presence of people and corresponding signal in the SSPD before the start of the RSPD, and the suggested relevance of mobility patterns for the lacking signal in the RSPD in this early period, neither proxy is devoid of influence from population density or mobility patterns. Furthermore, the correspondence between the logistic growth in the RSPD and the positive deviation in the SSPD could suggest that after However, this has to be left as very tenuous suggestions, as properly adjudicating this relationship will depend on independent data that tracks the developments of the other factors that hvae been suggested to be of relevance to the behaviour of the two proxies.  
...
Thus, while it appears that the SSPD and RSPD are differentially impacted by population density, and, we cautiously suggest, the SSPD is more heavily influenced by land-use and mobility patterns, this suggestion and the nature of the discrepancy between the proxies clearly requires further investigation. Some lines of inquiry could be to also draw on lithic technology and tool-kit diversity as a function of population size and density [e.g. @berg-hansen2018; @collard2013a; @riede2014; @solheim2020], and mobility patterns that can potentially be inferred from the composition of lithic assemblages [e.g. @barton2014; @clark2017; @preston2018; @roalkvam2022]. Environmental conditions, constraints and developments also represent a range of factors that can have implications for these dimensions, and will consequently be of future importance to unpack the relationship between the proxies [e.g. @jorgensen2020; @ordonez2022]. Aligning these different lines of evidence would also be dependent on a more direct modelling of the development in the SSPD, rather than simply basing the analysis on deviations from a rejected null model [cf. @timpson2021]. Furthermore, mapping and contrasting these developments across a larger region will not only offer a larger sample size, which is especially limited for the ^14^C-data, but could also provide important contextualisation for the developments observed here [see @lundstrom2021].

Finally, seeing as the method for shoreline dating implemented here was recently developed and has been subjected to limited evaluation and testing, there are still considerable uncertainties associated with its application. While we still believe the approach represents an improvement over previous approaches, the results should therefore considered a cautions first exploration of the implications of employing a probabilistic method for shoreline dating when evaluating large scale trends in the frequency of coastal sites in the region.
...
This paper has presented a first exploration of summing the results of using the probabilistic method for shoreline dating presented in @roalkvam2023a for assessing the development of the frequency of shoreline dated sites over time. The approach demonstrates that it 

The finding that the frequency of radiocarbon dates and frequency of shoreline dated sites, in general, follow trajectories is a valuable insight. Although a more precise understanding of this relationship
